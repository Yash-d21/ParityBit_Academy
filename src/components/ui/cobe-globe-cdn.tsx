...
export function GlobeCdn({
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    ...
    // NOTE: The inline style updating causes issues on scroll on some mobile layouts, 
    // replacing strict canvas grabs with css classes handles it cleaner, but this works:
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      // Adjust to ParityBit styling — dark violet glow/markers
      globe = createGlobe(canvas, {
        devicePixelRatio: Math.min(window.devicePixelRatio || 1, 2),
        width, height: width,
        phi: 0, theta: 0.2, dark: 0, diffuse: 1.5,
        mapSamples: 16000, mapBrightness: 8,
        baseColor: [0.98, 0.98, 0.98], // white/cream globe
        markerColor: [0.48, 0.17, 0.75], // #7B2CBF violet markers
        glowColor: [0.90, 0.88, 0.95],
        markerElevation: 0.02,
        markers: markers.map((m) => ({ location: m.location, size: 0.05, id: m.id })),
        arcs: arcs.map((a) => ({ from: a.from, to: a.to, id: a.id })),
        arcColor: [0.48, 0.17, 0.75], // Violet arcs
  ...
  const pyramidFaceStyle = (nth: number): React.CSSProperties => {
    ...
    // Violet/Purple hues for the pyramid tips instead of pure greyscale
    const colors = ["#4a1577", "#7b2cbf", "#9d4edd", "#c77dff"]
    return {
      ...
      {traffic.map((t) => (
        <div
          ...
          style={{
            ...
            fontSize: "0.5rem",
            color: "#fff",
            background: "#60A5FA", // Blue pop for traffic
            padding: "3px 8px",
            borderRadius: 4,
